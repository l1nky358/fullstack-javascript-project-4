...
describe('Page Loader - Error Handling', () => {
  ...
  test('should handle resource 404 error gracefully', async () => {
    ...
    expect(savedContent).toContain('/missing.jpg')
    expect(savedContent).not.toContain('_files')
  })
  ...
  test('should handle resource network error gracefully', async () => {
    ...
    const filepath = await pageLoader(url, tempDir)
    const savedContent = await fs.readFile(filepath, 'utf-8')
    expect(savedContent).toContain('/app.js')
  })
  ...
  test('should handle multiple resource failures', async () => {
    ...
    const filepath = await pageLoader(url, tempDir)
    const savedContent = await fs.readFile(filepath, 'utf-8')
    expect(savedContent).toContain('/img1.jpg')
    expect(savedContent).toContain('/img2.jpg')
    expect(savedContent).toContain('/style.css')
  ...
  test('should download successful resources and skip failed ones', async () => {
    ...
    const filepath = await pageLoader(url, tempDir)
    const savedContent = await fs.readFile(filepath, 'utf-8')
    expect(savedContent).toContain('example-com-page_files/example-com-good.jpg')
    expect(savedContent).toContain('example-com-page_files/example-com-styles.css')
    expect(savedContent).toContain('/bad.jpg')
